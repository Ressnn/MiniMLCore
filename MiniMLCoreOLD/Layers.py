from MiniMLCore import Neurons
import numpy as np
import cupy
...
class CudaDense(Layer):
    ...
    def build(self,input_shape):
        ...
        self.weights = cupy.random.normal(size=(self.units,input_shape))
        self.baises = cupy.random.normal(size=self.units)
        return self.units
    def compute(self,inputs):
        inputs = cupy.array(inputs)
        return cupy.dot(self.weights, inputs)+self.baises
    def calculate_gradients(self,inputs):
        part_gradients = dict()
        part_gradients["Weights"] = inputs
        part_gradients["Baises"] = cupy.ones(self.units)
        part_gradients["PrevLayer"] = self.weights
        return part_gradients
    def apply_changes(self,part_gradients):
        self.weights=part_gradients["Weights"]
        self.baises=part_gradients["Baises"]
